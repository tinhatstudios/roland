...
class diff_match_patch {
  ...
  /**
     ...
     */
  diff_main(text1, text2, opt_checklines,
    opt_deadline) {
  ...
  diff_compute_(text1, text2, checklines,
    ...
    if (i != -1) {
      ...
      diffs = [new Diff(DIFF_INSERT, longtext.substring(0, i)),
    ...
    if (hm) {
      ...
      // Merge the results.
      return diffs_a.concat([new Diff(DIFF_EQUAL, mid_common)],
        diffs_b);
    }
    ...
    for (var x = 0; x < tokens.length; x++) {
      ...
      switch (tokens[x].charAt(0)) {
        case '+':
          try {
            diffs[diffsLength++] =
              new Diff(DIFF_INSERT, decodeURI(param));
          } catch (ex) {
  ...
  patch_deepCopy(patches) {
    ...
    for (var x = 0; x < patches.length; x++) {
      ...
      for (var y = 0; y < patch.diffs.length; y++) {
        patchCopy.diffs[y] =
          new Diff(patch.diffs[y][0], patch.diffs[y][1]);
      }
